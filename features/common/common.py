...
# ````````````````````````````````````````基础操作封装````````````````````````````````````````
@step(u'用例编写者"(.*?)"，优先级"(.*?)"')
def record(step, owner, priority):
    pass
...
@step(u'我尝试点击"(.*?)"，如果不存在，将点击"(.*?)"后再次尝试，最多(.*)次')
...
@step(u'我尝试点击文本"(.*?)"，如果文本不存在，将点击"(.*?)"后再次尝试，最多(.*)次')
...
# ````````````````````````````````````````常用查询````````````````````````````````````````
# @step(u'我查询"(.*?)"是否存在，如果存在将"(.*?)"，如果不存在将什么都不做')
# def check_element_exist(step, element, operation):
#     element = world.element[element]
#     try:
...
@step(u'我操作手机设备，进行所有权限授权')
def check_device_permissions(step):
    step.behave_as(u'Given 我操作手机设备，进行GPS授权\n'
                   u'And 我操作手机设备，进行短信授权\n'
                   u'And 我操作手机设备，进行照片授权\n'
                   u'And 我操作手机设备，进行通讯录授权')
